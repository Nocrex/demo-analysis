    ...
    pub fn new() -> Self {
        Self {
            params: HashMap::from([
                ...
                ("min_tick".to_string(), Parameter::Int(20)),
                ("weapon_id".to_string(), Parameter::Int(16)),
            ]),
            shots: HashMap::new(),
            ..Default::default()
        }
    ...
    fn on_message(
        ...
        if let Message::GameEvent(event_msg) = message {
            if let GameEvent::PlayerHurt(hurt) = &event_msg.event {
                ...
                let min_tick: i32 = get_parameter_value(&self.params, "min_tick");
                let weapon_id: i32 = get_parameter_value(&self.params, "weapon_id");
                let weapon_mapping = HashMap::from([
                    (weapon_id as u32, min_tick as u32), // scout primary
                ]);

                let dmg = hurt.damage_amount as u32;

                if dmg == 4 { // ignore wrap assassin bleed
                    return Ok(vec![]);
                }
                ...
                if past_weapon != weapon ||
                    past_victim != victim_uid {
                    return Ok(vec![]);
                }
